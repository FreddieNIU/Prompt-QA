...
def read_dataset(path, have_template=True):
    ...
    return contexts, questions, answers, positions


def add_token_positions(encodings, tokenizer, contexts, answers, positions):
        ...
        for i in range(start_position, len(context_tokens)):
            ...
            for j in range(len(answer_tokens)):
                if answer_tokens[j] != context_tokens[i + j]:
                    break
                new_end = i + j +1
            if new_end - i  == len(answer_tokens):
                return i, new_end
    ...
    start_positions = []
    end_positions = []
    context_encodings = tokenizer(contexts, truncation=True)
    for i in range(len(contexts)):
        start_in_context = context_encodings.char_to_token(i,answers[i]['answer_start'])
        end_in_context = context_encodings.char_to_token(i,answers[i]['answer_end']-1)
        if start_in_context is None:
            start_in_context =  tokenizer.model_max_length
        if end_in_context is None:
            end_in_context = tokenizer.model_max_length
        answer_tokens = tokenizer.convert_ids_to_tokens(context_encodings['input_ids'][i])[start_in_context:end_in_context+1]
        context_tokens = tokenizer.convert_ids_to_tokens(encodings['input_ids'][i])
        prev_start = positions[i]['start_position']
        prev_end = positions[i]['end_position']
        if answer_tokens:
            start_position, end_position = improve_answer_span(context_tokens, answer_tokens, 0, 0)
        else:
        ...
        # print(tokenizer.convert_ids_to_tokens(encodings['input_ids'][i])[start_position:end_position])
        # print(answer_tokens)
...
if __name__ == '__main__':
    ...
    for config in configs:
        ...
        # LP fine tune
        best_dev_f1 = train(LP_model, num_epoch=1, SAVE=False, lr=1e-6)
        ...
        LPFT_model = AutoModelForQuestionAnswering.from_pretrained(config.model)
