...
def read_dataset(path):
    ...
    for qa in qa_dataset:
        ...
        # context = 'context: ' + qa['context'] +' '+ str([template[i] for i in range(len(template))])
...
def train(model, num_epoch=1, SAVE=True, lr=1e-5):
    ...
    for epoch in range(num_epoch):
        for question_batch, answer_batch in tqdm(train_loader):
            ...
            writer.add_scalar('LPFT_{}_{}_{}_loss'.format(config.source_domain, config.source_domain, config.source_template), loss, epoch)    
...
if __name__ == '__main__':
    ...
    for config in configs:
        ...
        best_dev_f1, linear_params = train(LP_model, num_epoch=3, SAVE=False, lr=1e-6)
